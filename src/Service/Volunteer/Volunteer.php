...
abstract class Volunteer extends Service
{
    ...
    {
        if ($required && !$this->hasRequiredPresentFields($data, ['organization', 'position', 'startDate', 'endDate', 'summary', 'highlights'])) {
            return false;
        }
    ...
    protected function getModelById(string $id): Volunteers
    {
        $model = Volunteers::first(
            [
                [
                    'id',
                    '=',
    ...
    /**
     * @param array<int, Volunteers>|bool $rows
     ...
     * @return array<string, mixed>
     */
    protected function parseModel(Volunteers $model): array
    {
        $data = $model->toArray();
