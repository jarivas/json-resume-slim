...
abstract class Work extends Service
{
    ...
    {
        if ($required && !$this->hasRequiredPresentFields($data, ['name', 'position', 'startDate', 'endDate', 'summary', 'highlights'])) {
            return false;
        }
    ...
    protected function getModelById(string $id): Works
    {
        $model = Works::first(
            [
                [
    ...
    /**
     * @param array<int, Works>|bool $rows
     ...
     * @return array<string, mixed>
     */
    protected function parseModel(Works $model): array
    {
        $data = $model->toArray();
